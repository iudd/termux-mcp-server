...
# =============================================================================
# Termux MCP Server 更新脚本 (无备份版本，修复PM2和警告)
# 适用于已安装环境的代码更新，直接在原目录修改
# 作者: MiniMax Agent
# 版本: 1.0.4 (修复rsync依赖)
# =============================================================================
...
# 错误处理函数
handle_error() {
    log_error "更新过程中发生错误，请检查上面的错误信息"
    log_info "常见解决方案:"
...
echo "======================================================================"
echo "                🔄 Termux MCP Server 无备份更新脚本 🔄"
echo "======================================================================"
echo ""
...
if [ -d "$PROJECT_DIR" ] && [ -f "$PROJECT_DIR/src/server.js" ] && [ -f "$PROJECT_DIR/package.json" ]; then
    IS_INSTALLED=true
    log_success "检测到现有安装: $PROJECT_DIR"

    # 检查服务是否正在运行
    if pgrep -f "termux-mcp-server" >/dev/null 2>&1 || pgrep -f "src/server.js" >/dev/null 2>&1; then
        IS_RUNNING=true
        log_info "检测到服务正在运行"
    else
        IS_RUNNING=false
...
EOF
    ...
    if [ "$IS_RUNNING" = true ]; then
        log_info "停止当前运行的服务..."
        if check_command "pm2"; then
            pm2 stop termux-mcp-server 2>/dev/null || true
            pm2 delete termux-mcp-server 2>/dev/null || true
        else
            pkill -f "src/server.js" 2>/dev/null || true
            sleep 2
    ...
    # 下载最新代码
    log_info "下载最新代码..."
    TEMP_DIR="$PROJECT_DIR/temp-update-$$"
    mkdir -p "$TEMP_DIR"
    ...
    if [ "${DOWNLOAD_SUCCESS:-false}" = true ]; then
        ...
        if tar -xzf "$TEMP_DIR/project.tar.gz" -C "$TEMP_DIR/extracted" --strip-components=1 2>/dev/null; then
            # 排除配置文件和日志目录
            rsync -av --exclude='.env' \
                     ...
                     --exclude='.git/' \
                     "$TEMP_DIR/extracted/" "$PROJECT_DIR/"

            # 检查npm依赖是否有变化
            if ! diff -q "$TEMP_DIR/extracted/package.json" "$PROJECT_DIR/package.json" >/dev/null 2>&1; then
                log_info "检测到依赖变化，更新npm包..."
                if npm install --silent; then
                    log_success "依赖更新完成"
                else
                    log_warning "依赖更新失败，继续使用现有版本"
                fi
            fi

            log_success "代码更新完成（配置文件已保留）"
        else
            log_error "代码解压失败"
            exit 1
        fi
    ...
    if check_command "pm2" && [ -f "ecosystem.config.js" ]; then
        pm2 start ecosystem.config.js --env production
        sleep 3
        if pm2 list | grep -q "termux-mcp-server.*online"; then
            START_METHOD="pm2"
            log_success "服务已使用PM2重启"
        else
            log_warning "PM2重启失败，尝试直接启动"
            pkill -f "src/server.js" 2>/dev/null || true
            sleep 2
            nohup node src/server.js > server.log 2>&1 &
            SERVER_PID=$!
            ...
            if kill -0 "$SERVER_PID" 2>/dev/null; then
                START_METHOD="direct"
                echo "$SERVER_PID" > server.pid
                log_success "服务已直接重启，PID: $SERVER_PID"
            else
                log_error "服务重启失败"
                exit 1
            fi
...
# 获取服务运行时间
if [ "${START_METHOD:-unknown}" = "pm2" ]; then
...
echo "├─ 启动方式: ${START_METHOD:-unknown}"
echo "├─ 端口: 3001"
echo "├─ 运行时间: $UPTIME"
...
echo "└─ 局域网访问: http://$(hostname -I 2>/dev/null | awk '{print $1}' || echo "未知"):3001"
echo ""
echo "🔧 管理命令:"
...
if [ "$IS_INSTALLED" = true ]; then
    echo "⚠️  更新提示:"
    echo "• 代码已更新到最新版本"
    echo "• 配置文件(.env, ecosystem.config.js)和日志已保留"
    echo "• node_modules目录已保留，避免重复下载"
    echo "• 如有问题请查看日志"
else
    echo "⚠️  重要提示:"
    echo "• 服务器已配置为生产环境模式"
    echo "• 建议定期更新代码以获取最新功能"
fi
echo ""
